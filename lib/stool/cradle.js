...
Stool.prototype.iterate = function (startKeyDocId, endKeyDocId, pageSize, numPages,
        processCb, finishCb, successCb, errorCb) {
    var that = this,
    ...
    this.database.all(params, function (err, result) {
        ...
        that.pageNum += 1;
        console.log('Processing page ' + that.pageNum + ': ' + pageSize +
            ' docs from ID ' + startKeyDocId);
        processCb(result);
        // continue to next page
        if ((numPages === -1 || that.pageNum < numPages) && result.length === pageSize + 1) {
            startKeyDocId = result[result.length - 1].doc._id;
            that.iterate(startKeyDocId, endKeyDocId, pageSize, numPages,
                processCb, finishCb, successCb, errorCb);
        // last page
        } else {
