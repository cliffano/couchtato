...
// opts: { startKeyDocId, endKeyDocId, pageSize, numPages }
Stool.prototype.iterate = function (opts, processCb, finishCb, successCb, errorCb) {
    var that = this,
        params = { include_docs: true, limit: opts.pageSize + 1,
            startkey_docid: opts.startKeyDocId};
    if (opts.endKeyDocId) { params.endkey_docid = opts.endKeyDocId; }
    this.database.all(params, function (err, result) {
        if (err) { throw new Error(err.error + ' - ' + err.reason); }
        that.pageNum += 1;
        console.log('Processing page ' + that.pageNum + ': ' + opts.pageSize +
            ' docs from ID ' + opts.startKeyDocId);
        processCb(result);
        // continue to next page
        if ((opts.numPages === -1 || that.pageNum < opts.numPages) &&
                result.length === opts.pageSize + 1) {
            opts.startKeyDocId = result[result.length - 1].doc._id;
            that.iterate(opts, processCb, finishCb, successCb, errorCb);
        // last page
        } else {
